...
exports.showSetting = function (req, res, next) {
  User.getUserById(req.session.user._id, async function (err, user) {
    if (err) {
      return next(err);
    }
    ...
    let data =  await volunteers.findOne({login:req.session.user._id});
    if(data){
      data.phone = data.phone.substr(0, 3) + '****' + data.phone.substr(7);
    }

    user.error = null;

    user.volunteers = data;
